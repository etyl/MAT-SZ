...
def make_predictor(args, img: np.ndarray):
    return InterpPredictor("cubic", args.levels, args.anchor_stride, args.anchor_block)


def build_predictor_for_decompress(args, hdr: Header):
    return InterpPredictor("cubic", hdr.levels, hdr.anchor_stride, hdr.anchor_block)
...
def main():
    ...
    for img_path in images:
        ...
        for eb in sorted(args.eb):
            ...
            print(
                f"[{done:3d}/{total}] {img_path.name}  eb={eb} ...", end=" ", flush=True
            )
            try:
