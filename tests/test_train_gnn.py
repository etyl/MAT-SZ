...
def test_synthetic_batch_is_4d_normalized_and_reproducible():
    ...
    a = sample_synthetic_batch(
        2, shape, correlation, np.random.default_rng(123))
...
def test_synthetic_fields_randomly_permute_correlation_axes():
    shape = (16, 16, 16, 16)
    fields = sample_synthetic_batch(
        6, shape, (6.0, 3.0, 1.5, 0.5),
        np.random.default_rng(19)).reshape(6, *shape)
    ...
    smoothest_axes = []
    for field in fields:
        variation = [float(torch.diff(field, dim=axis).abs().mean())
                     for axis in range(4)]
...
def test_mixed_batch_fraction_counts_scalar_points():
    image_batch, synthetic_batch, actual = mixed_batch_sizes(
        crop=128, synthetic_shape=(16, 16, 16, 16),
        synthetic_fraction=0.25, image_batch=8, synthetic_batch=1)
...
def test_chunk_finalization_casts_autocast_output_to_field_dtype():
    ...
    with torch.autocast(device_type="cpu", dtype=torch.bfloat16):
        nll, npix, _ = run_chunked_scene(
            model, x, (8, 8), axis=1, order=(0, 1), levels=2, stride=4,
            d=8, device=torch.device("cpu"), eb=0.01, agg_level=2)
    loss = nll / npix
    loss.backward()
